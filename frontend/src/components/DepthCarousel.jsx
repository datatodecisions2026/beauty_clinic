...
// Adapted from React Bits "DepthCarousel". The depth-rail motion is unchanged;
// each card renders a review ({ text, author, rating, relative_time, photo, uri })
// when the item has `text`, and falls back to an <img> for image items.
...
const Stars = ({ rating = 5 }) => {
  ...
  return (
    <span className="depth-carousel__stars" aria-hidden="true">
      {'★'.repeat(r)}
      <span className="depth-carousel__stars-off">{'★'.repeat(5 - r)}</span>
    </span>
  );
};
...
const DepthCarousel = ({
  ...
  const isReviews = data.some(d => typeof d.text === 'string' && d.text);
  ...
  return (
    <div
      ref={rootRef}
      className={`depth-carousel ${isReviews ? 'depth-carousel--reviews ' : ''}${className}`.trim()}
      style={{ '--dc-perspective': `${perspective}px` }}
      role="group"
      aria-roledescription="carousel"
      aria-label={isReviews ? 'Client reviews' : 'Depth carousel'}
      tabIndex={0}
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => {
          const review = typeof item.text === 'string' && item.text;
          return (
            <div
              key={i}
              ...
              aria-label={review ? `Review by ${item.author || 'client'}` : `${i + 1} of ${count}`}
              aria-hidden={active !== i}
              onClick={() => onCardClick(i)}
            >
              {review ? (
                <div className="depth-carousel__review">
                  <span className="depth-carousel__gmark" aria-hidden="true">G</span>
                  <Stars rating={item.rating} />
                  <p className="depth-carousel__quote">{item.text}</p>
                  <span className="depth-carousel__meta">
                    {item.photo ? (
                      <img className="depth-carousel__avatar" src={item.photo} alt="" loading="lazy" referrerPolicy="no-referrer" />
                    ) : (
                      <span className="depth-carousel__avatar depth-carousel__avatar--initial" aria-hidden="true">
                        {(item.author || '?').trim().charAt(0).toUpperCase()}
                      </span>
                    )}
                    <span className="depth-carousel__who">
                      <span className="depth-carousel__name">{item.author}</span>
                      {item.relative_time && <span className="depth-carousel__when">{item.relative_time}</span>}
                    </span>
                  </span>
                  {item.uri && (
                    <a
